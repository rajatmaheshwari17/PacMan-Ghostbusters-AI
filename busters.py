...
def readCommand(argv):
    ...
    usageStr = """
    USAGE:      python3 busters.py <options>
    EXAMPLES:   (1) python3 busters.py -p GreedyBustersAgent -l smallHunt
                    - starts an interactive game on a small hunt layout
                (2) python3 busters.py --frameTime 0
                    - starts an interactive game with no pause between moves
    """
    parser = OptionParser(usageStr)

    parser.add_option('-p', '--pacman', help='Pacman agent type (default %default)',
                      choices=['KeyboardAgent', 'GreedyBustersAgent', 'BustersKeyboardAgent'], default='KeyboardAgent')
    parser.add_option('-l', '--layout', help='Maze layout file from which to load the map layout (default %default)',
                      default='layouts/smallHunt')
    parser.add_option('-g', '--ghosts', help='Ghost agent type (default %default)', default='RandomGhost')
    parser.add_option('-k', '--numghosts', type='int', help='The maximum number of ghosts to use (default %default)',
                      default=4)
    parser.add_option('--frameTime', type='float', help='Time to delay between frames; <0 means keyboard (default %default)',
                      default=0.1)
    parser.add_option('-q', '--quietTextGraphics', action='store_true',
                      help='Display output as text only (no graphics)')
    parser.add_option('--seed', type='int', help='Random seed (default %default)', default=None)
    ...
    # Choose a Pacman agent
    agentOpts = {}
    pacmanType = globals().get(options.pacman) or bustersAgents.__dict__.get(options.pacman)
    if pacmanType is None:
        raise Exception('Unknown agent type: ' + options.pacman)

    # Initialize Pacman
    if options.pacman == 'KeyboardAgent':
        pacman = pacmanType()  # KeyboardAgent requires no extra options
    else:
    ...
    # Choose a ghost agent
    ghostType = globals().get(options.ghosts) or bustersAgents.__dict__.get(options.ghosts)
    if ghostType is None:
        raise Exception('Unknown ghost type: ' + options.ghosts)
    ...
    if options.quietTextGraphics:
        ...
        args['display'] = PacmanGraphics(frameTime=options.frameTime)
