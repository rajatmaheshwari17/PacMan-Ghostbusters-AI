...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {
        ...
        OBS0: list(range(MAX_MANHATTAN_DISTANCE + MAX_NOISE + 1)),
        OBS1: list(range(MAX_MANHATTAN_DISTANCE + MAX_NOISE + 1))
    }
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        factors = list(bayesNet.getAllCPTsWithEvidence(evidenceDict))
    
        for variable in eliminationOrder:
            factors_without_var, joined_factor = joinFactorsByVariable(factors, variable)

            if joined_factor is None:
                factors = factors_without_var
                continue

            if len(joined_factor.unconditionedVariables()) == 1 and variable in joined_factor.unconditionedVariables():
                factors = factors_without_var
                continue
                
            if variable in joined_factor.unconditionedVariables():
                eliminated_factor = eliminate(joined_factor, variable)
                if eliminated_factor is not None:
                    factors = factors_without_var + [eliminated_factor]
                else:
                    factors = factors_without_var
            else:
                factors = factors_without_var + [joined_factor]

        if len(factors) == 0:
            raise ValueError("No factors remaining")
        elif len(factors) == 1:
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key, threshold in cumulative:
            if rand <= threshold:
                return key
